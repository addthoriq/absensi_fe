...
<div class="d-flex justify-content-center">
    <div class="card p-5">
        <div class="mx-5 d-flex flex-column gap-2">
            ...
            {:else}
                <form method="POST" action="/dashboard/attendances/in" class="d-flex justify-content-center flex-column gap-2">
                    <input type="hidden" name="lat" value={lat}>
                    <input type="hidden" name="long" value={long}>
                    <button type="submit" class="btn btn-info">Simpan Kehadiran Masuk</button>
                </form>
            {/if}
